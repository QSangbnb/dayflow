...
The project uses the Orbio OpenAI-compatible API entirely on the server. A single request can combine:
...
Node.js HTTP server
  ...
  └─ secret-key redaction
          │
          ▼
Orbio API gateway → models and tools
```

The browser never receives `ORBIO_API_KEY`. Uploaded content is passed to the inference request and is not written to disk or a database by this application.
...
```env
ORBIO_API_KEY=your_sk_orbio_key
...
DayFlow is the public Orbio Build Week entry.
